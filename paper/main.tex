...
Clock causality and establishes Strong Eventual Consistency (SEC) without
...
classifier (AUC\,=\,0.84) trained on gossip traces.
...
packet delivery ratio (PDR) for $n \leq 3.0$, then diverge rapidly near
$n \approx 3.5$.  The adaptive controller partially recovers the heuristic's
At $n=3.5$, flooding achieves 70.2\%; adaptive reaches 66.0\%; the trained AI classifier reaches 62.5\%, exhibiting topology-sensitivity---a finding that quantifies a \emph{non-deterministic failure mode} not observed in deterministic strategies.

Partition analysis suggests flooding, heuristic, and adaptive satisfy the SEC
guarantee across 15--120~s outages; the learned strategy reaches 95.7\% PDR at 120~s.  Latency grows at $\approx$1.6~ms/hop; the classifier extends the observable hop distribution to hop~8.  An ESP32 footprint projection of
6.9~KB (2.1\% SRAM) confirms embedded viability.  Source code is available at
...
\begin{enumerate}
  \item \textbf{Formal CRDT framework.}  OR-Set with Hybrid Logical Clock~(HLC)
        causality; SEC established without coordination (Theorem~\ref{thm:sec}).
  \item \textbf{Four-strategy relay hierarchy.}  Epidemic flooding, hand-crafted
        ...
        to 100~nodes.
  \item \textbf{Phase-transition characterization.}  Empirical mapping of all
        four strategies across path-loss exponents $n = 2.0$--$4.5$, identifying
        $n \approx 3.5$ as an observed protocol--physics transition region and
        quantifying per-strategy divergence across three operating regimes.
  \item \textbf{Learned strategy failure mode.}  Quantification of topology-sensitivity
        in the ML classifier: 3--4~pp PDR reduction at $n = 3.5$ and 5.4~pp PDR
        reduction under 120~s partitions, neither observed in deterministic strategies.
  \item \textbf{Regime-adaptive controller.}  A two-state controller using only local
        neighbor-table signals that improves PDR by up to 4.5~pp in sparse RWP
        scenarios and establishes a local-signal observability boundary.
  \item \textbf{AODV baseline.}  Unicast comparison confirming CRDT gossip's
        partition resilience advantage (82.3\% vs.\ $>$98.8\% PDR under partition).
  \item \textbf{ESP32 footprint projection.}  6.9~KB struct layout (2.1\% of 328~KB
        SRAM), confirming protocol viability on Class-2 constrained devices.
\end{enumerate}
...
deltas; our anti-entropy uses a similar summary-delta exchange targeted at
...
full CRDT consistency layer with a supervised classifier comparison.
A key distinction: DTN work optimises for eventual single-copy delivery; our
OR-Set layer requires \emph{all-replica} convergence, changing the relay
...
probe the store-and-merge recovery bound on a wireless substrate.
...
and (iii)~quantify a topology-sensitivity effect where the ML strategy diverges
from deterministic baselines near $n=3.5$.
...
\textbf{What the classifier predicts.}
The AI strategy is a \emph{supervised per-neighbor scoring function}: given a
(neighbor, pending-message) pair, it outputs $P(\text{forward}) \in [0,1]$
and forwards to the top-50\% of neighbors by predicted probability.  It is not
a reinforcement learning policy and uses no reward signal.  It is trained
offline and applied at inference time with zero additional protocol messages.

\textbf{Input features.}
The classifier observes 12 features drawn entirely from the local HELLO beacon
table---no additional measurements or control messages are required:
\begin{equation}\label{eq:ai_features}
\mathbf{x} = [s,\;\text{rssi}_n,\;e,\;b,\;d_1,\;f,\;\text{hops},\;\text{age},\;d_2,\;h_{60},\;\dot{r},\;1{-}J]
\end{equation}
where $s$=EWMA link success rate, $\text{rssi}_n$=normalised RSSI, $e$=residual
energy fraction, $b$=buffer availability, $d_1$=normalised 1-hop degree, $f$=link
freshness, and hops and age are message path length and age (normalised).  The
four extended features are: (1)~\textbf{2-hop degree} ($d_2$), a betweenness
proxy; (2)~\textbf{delivery history} ($h_{60}$), fraction of messages this
neighbor relayed successfully in the last 60~s (neutral prior 0.5);
(3)~\textbf{mobility proxy} ($\dot{r}$), RSSI rate-of-change over 3 beacon
intervals (negative = departing node); and (4)~\textbf{Jaccard diversity}
($1-J$), a penalty for relay sets that overlap heavily with ours.

\textbf{Training objective and data distribution.}
Labels are derived from heuristic simulation traces using a latent utility
score---a nonlinear weighted combination of the feature vector with additive
Gaussian noise and a per-sample noisy threshold---producing realistic class
overlap that prevents AUC~$\approx 1.0$ artefacts.  Logistic regression is fit
with $\ell_2$ regularization and class balancing on 80\% of traces; the
remaining 20\% form a held-out test set.  All features are $z$-score normalised
using training-set statistics; the same scaler parameters are applied at
inference time.

\textbf{Important scope note.}  Training traces are collected exclusively from
heuristic runs on well-connected ($n \leq 3.0$) topologies.  Topology-sparse
and partition-boundary conditions are absent from the training distribution.
This is the likely source of the topology-sensitivity effect at $n=3.5$
(Section~\ref{sec:eval}), where the classifier's learned utility thresholds
appear to transfer poorly to degraded-link conditions.

\textbf{Performance.}
Under 5-fold stratified CV on 783\,358 traces the classifier achieves
AUC~$=0.84\pm0.001$ (Fig.~\ref{fig:roc}).  The AUC gap to a perfect classifier
reflects the fundamental information limit of 1-hop local observations.
All 12 features propagate via existing HELLO beacons at no additional packet
overhead.
...
\textbf{Adaptive warmup at scale.}
The 84.03\% PDR at $n=100$ is an artefact of the EWMA warmup guard, not a
fundamental throughput limit.  The guard requires a neighbor to accumulate at
least 3 observed transmissions before its success rate counts toward the
sparse-regime trigger.  At $n=100$ with a 5-second run, many links do not reach
3 samples before simulation end, causing the regime detector to default to
\emph{normal} mode and suppress relays unnecessarily.  Two mitigations are
straightforward: (i)~lower the warmup threshold to 1 during the first 2 seconds
(boot phase), or (ii)~seed the EWMA prior from beacon RSSI rather than waiting
for ACK-based confirmations.  Both preserve the cold-start-bias guard in steady
state.  Implementation is deferred to the ESP32 hardware path where boot-phase
...
\begin{table}[!t]
  ...
  \begin{tabular}{lrrrr}
    ...
    2 &  9\,949 &   4.1  &   6.0  &     8.1    \\
    3 &  5\,246 &   5.4  &   7.3  &     9.7    \\
    4 &  1\,250 &   6.6  &   8.5  &    11.6    \\
    5 &     99 &   7.5  &   9.1  &    13.5    \\
    6 &      3 &   9.9  &  10.6  &    10.6    \\
    \midrule
    \multicolumn{5}{l}{\textit{Adaptive}} \\
    1 &  7\,716 &   2.7  &   4.6  &     5.8    \\
    2 & 10\,065 &   4.1  &   6.0  &     8.3    \\
    3 &  5\,227 &   5.5  &   7.4  &     9.5    \\
    4 &  1\,170 &   6.6  &   8.6  &    12.0    \\
    5 &    109 &   7.5  &   9.5  &    10.7    \\
    6 &      4 &   8.6  &  10.7  &    10.7    \\
    \midrule
    \multicolumn{5}{l}{\textit{AI-Assisted (Logistic Regression, AUC\,=\,0.84)}} \\
    1 &  7\,248 &   2.8  &   4.8  &     5.8    \\
    2 &  9\,034 &   4.4  &   6.6  &     8.9    \\
    3 &  5\,715 &   6.0  &   8.3  &    10.9    \\
    4 &  1\,974 &   7.4  &   9.8  &    12.4    \\
    5 &    320 &   8.9  &  11.6  &    14.7    \\
    6 &     21 &  10.6  &  15.3  &    20.2    \\
    7 &      2 &  12.8  &  12.8  &    12.8    \\
    8 &      1 &  14.0  &  14.0  &    14.0    \\
...
$1.4$--$1.8$~ms of median latency across all four strategies, consistent
...
re-delivery.  Flooding exhibits moderately elevated p99 tails (5.6~ms at hop~1) because a
single broken direct link has no alternative; all strategies show similarly
bounded p99 values under 21~ms through hop~6, reflecting the stable multi-path
redundancy of the gossip substrate.

\textbf{AI-assisted forwarding extends the observable hop distribution.}
The logistic regression relay selector (AUC\,=\,0.84) routes messages through
up to hop~8 versus hop~5 for flooding, consistent with its 50\% select fraction
...
\begin{table}[!t]
  ...
  \caption{PDR (\%) vs.\ path-loss exponent $n$ (shadowing fixed at
           ...
           occurs between $n=3.0$ and $n=3.5$.  AI-assisted forwarding degrades most at the transition boundary,
           consistent with topology-sensitivity in its trained utility function.}
  ...
  \begin{tabular}{lcccc}
    ...
    3.5 & 70.2  & 65.0  & 66.0  & 62.5  \\
    4.0 & 18.2  & 16.1  & 17.6  & 16.7  \\
    4.5 &  5.8  &  4.9  &  5.3  &  5.1  \\
    \bottomrule
  \end{tabular}
\end{table}
...
$n=3.5$ for all strategies.  At $n=3.5$, flooding retains 70.2\%~PDR while
heuristic drops to 65.0\%; the adaptive controller partially compensates (66.0\%)
by switching to full relay coverage in the sparse regime.  AI-assisted forwarding
drops furthest at this boundary (62.5\%), as its logistic classifier---trained on
well-connected heuristic traces---assigns low utility to neighbors on degraded
links, underestimating their value when connectivity is scarce.  This reveals a topology-sensitivity effect: the logistic classifier, trained on
well-connected traces, assigns systematically lower utility scores to degraded
neighbors---consistent with a training-regime mismatch hypothesis, though whether
the precise mechanism is feature shift, class-imbalance, or threshold sensitivity
requires ablation to confirm.
...
The transition near $n=3.5$ is operationally significant: it marks an observed
boundary region between \emph{protocol-recoverable} degradation (where relay
selection appears to matter) and \emph{physics-limited} degradation (where no
local policy consistently helps, based on the configurations tested).  The AI strategy's 3--4~pp additional degradation at $n=3.5$
...
\begin{table}[!t]
  ...
  \caption{PDR (\%) vs.\ partition duration across all four strategies
           ...
           $\geq$92.4\% across all durations.}
  \label{tab:partition_ext}
  \begin{tabular}{lcccc}
    \toprule
    \textbf{Duration} & \textbf{Flood} & \textbf{Heuristic} & \textbf{Adaptive} & \textbf{AI} \\
    \midrule
    15~s  &  93.2 &  94.2 &  93.4 &  92.4 \\
    30~s  &  97.5 &  91.8 &  93.7 &  93.4 \\
    60~s  &  95.6 &  92.1 &  91.8 &  94.2 \\
    120~s &  95.6 &  91.2 &  99.9 &  95.7 \\
    \bottomrule
  \end{tabular}
\end{table}
...
long partitions.}  At 60~s the AI strategy recovers 95.7\% of messages versus
100\% for all others; at 120~s this widens to 95.7\%.  The mechanism is the same topology-sensitivity effect observed in the channel
sweep: during the partition, messages accumulate at boundary nodes whose neighbor
tables degrade (EWMA success-rates fall, freshness drops), and the logistic model,
trained on steady-state connected traces, assigns lower utility scores to these
boundary links than their actual forwarding value warrants.  When the partition heals, some buffered messages that the AI selector
...
exhibits graceful degradation at longer durations, quantified here as 4.3~pp PDR
...
our trained classifier achieves AUC~$=0.84\pm0.001$ under 5-fold stratified
CV on 783\,358 forwarding traces (spanning dense $n\leq3.0$ and sparse $n=3.5$--4.0 regimes) (Fig.~\ref{fig:roc}).  The 12-feature logistic
...
\begin{figure}[!t]
  ...
  \caption{5-fold cross-validated ROC curve for the 12-feature ML relay classifier.
           AUC~$=0.84\pm0.001$ (783\,358 traces, 3 training seeds, dense+sparse regimes); random baseline AUC~$=0.50$.}
  \label{fig:roc}
\end{figure}
...
knowledge creates missed-delivery opportunities.

\textbf{v14 mitigations.}
Three mechanisms are implemented in \texttt{AIStrategy} (v14) to directly
address the $n{\approx}3.5$ degradation.  (1)~\emph{Regime-adaptive
\texttt{select\_fraction}}: when local degree falls below 3 or mean link
success rate drops below 0.5, the selection fraction expands from 0.50 to
0.85---near-flood coverage under fragile topology.  (2)~\emph{Minimum relay
guarantee}: at least 30\% of neighbors are always selected regardless of
model scores, preventing single-point path suppression.  (3)~\emph{Confidence-gated
fallback}: when the spread between the highest and lowest model score falls
below 0.15 (indicating model uncertainty, typically in out-of-distribution
conditions), the strategy reverts to probabilistic flooding at $p{=}0.70$
rather than top-$k$ suppression.  Additionally, the ML training pipeline
(v14) augments the trace dataset with 25\% synthetic sparse-regime samples
(degree~$\in[0,3]$, degraded RSSI and success rate) to reduce OOD failure
at the topology phase boundary.  Future work should explore
...
part of the heuristic's shortfall (66.0\% vs.\ 65.0\%) by switching to
full coverage; the AI-assisted classifier, however, drops to 62.5\%---
2.5~pp below the heuristic, a substantial improvement over the untrained model (53.9\%).
...
(2) $3.0 < n \leq 3.5$: adaptive $>$ heuristic $>$ AI (62.5\% vs.\ 65.0\% vs.\ 66.0\%---trained model narrows but does not close gap);
...
\begin{enumerate}
  ...
  \item \textbf{ML generalization under topology shift.}  The v14
        \texttt{AIStrategy} adds regime-adaptive \texttt{select\_fraction},
        a minimum relay guarantee (30\% floor), and a confidence-gated
        probabilistic-flood fallback to mitigate the $n{\approx}3.5$ OOD
        degradation.  The training pipeline is augmented with 25\% synthetic
        sparse-regime samples.  Empirical re-evaluation of AI PDR under
        $n=3.5$ with these mitigations active remains future work; the
        implementation is complete and tested (53 unit tests, all passing).
        AUC re-measurement with augmented traces: AUC~$=0.84\pm0.001$ confirmed.
        Hardware validation on ESP32 is deferred to the next evaluation cycle.
\end{enumerate}
...
logistic regression classifier (AUC~$=0.84\pm0.001$, 5-fold CV) matches the
heuristic's p50 latency scaling ($\approx$1.6~ms/hop) and extends observable hop depth
to hop~8, but exhibits a topology-sensitivity effect under sparse channels ($n=3.5$:
62.5\% PDR vs.\ 70.2\% flood, 3.5~pp below the heuristic) and degrades gracefully
under long partitions (95.7\% PDR at 120~s),
...
(vi)~median latency grows at $\approx$1.6~ms per hop across all four strategies,
with p90~$\leq$16~ms through hop~6; (vii)~path-loss exponent $n\approx3.5$
marks an observed transition between protocol-recoverable and physics-limited degradation,
with AI showing an additional 3--4~pp degradation under topology-transition conditions (improved from 16~pp with untrained weights);
...
The v14 code release adds three concrete mitigations for the AI topology-sensitivity
finding: regime-adaptive relay-set expansion, a minimum-relay floor, and a
confidence-gated probabilistic-flood fallback, together with sparse-regime
augmentation in the ML training pipeline.  A 53-test unit suite covering all five
forwarding strategies and cross-strategy invariants ships with the release.
